...
from src.utils import load_dataset, model_train, evaluate_classification, calculate_adj_expected_score_diff
from src.regular_time_baseline_model.utils import MODEL_VERSION, MODEL_DRIVE, MODEL, EXPERIMENT_NAME, \
                                                    hyperparams, featureNames, num_fields_drive, num_fields, cat_fields
...
if __name__ == "__main__":
    ...
    with mlflow.start_run() as run:
    ...
    #if True:
        features_train = load_dataset('features_win_prob_reg_baseline_train')[featureNames]
        label_train = load_dataset('label_win_prob_reg_baseline_train').values.ravel()
        ...
        calculate_adj_expected_score_diff(drive_outcome_p, features_train)
        ...
        features_test = load_dataset('features_win_prob_reg_baseline_test')[featureNames]
        label_test = load_dataset('label_win_prob_reg_baseline_test').values.ravel()

        drive_outcome_p = model_drive.predict_proba(features_test)

        calculate_adj_expected_score_diff(drive_outcome_p, features_test)
