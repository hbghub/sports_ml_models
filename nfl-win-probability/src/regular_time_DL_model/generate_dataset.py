...
from src.regular_time_DL_model.utils import featureNames, num_fields, cat_fields
...
def generate_features(win_df, training_end_year):
    featureNames.extend(['season', 'week', 'offense_team'])
    features = win_df[featureNames]

    #train-test split
    id_train = (win_df.season <= training_end_year).tolist()
    id_test  = (win_df.season > training_end_year).tolist()
...
def generate_dataset_local():
    ...
    win_df.loc[id, 'offense_favorite_points'] = win_df.favorite_points[id] * (-1)

    features_train, features_test, label_train, label_test = generate_features(win_df, TRAINING_END_SEASON)

    save_dataset(features_train, "features_win_prob_reg_DL_train")

    save_dataset(features_test,  "features_win_prob_reg_DL_test")

    save_dataset(label_train, "label_win_prob_reg_DL_train")

    save_dataset(label_test,  "label_win_prob_reg_DL_test")
