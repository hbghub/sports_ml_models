...
from src.CFB_overtime_baseline_model.utils import featureNames, TRAINING_END_SEASON
...
def generate_features(win_df, training_end_season):
    ...
    label_train = win_df.loc[id_train, 'drive_outcome']
    label_test  = win_df.loc[id_test, 'drive_outcome']
...
def generate_dataset_local():
    ...
    save_dataset(features_train, "features_win_prob_CFB_ot_train")

    save_dataset(features_test,  "features_win_prob_CFB_ot_test")

    save_dataset(label_train, "label_win_prob_CFB_ot_train")

    save_dataset(label_test,  "label_win_prob_CFB_ot_test")
