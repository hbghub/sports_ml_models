...
from src.team_pass_ratio.utils import featureNames
...
def generate_features(game_df):
    ...
    # (4)
    # for pass ratio
    features_pass_ratio = game_df[featureNames]
    print(features_pass_ratio.shape)

    id = ~features_pass_ratio.isna().any(axis=1)
    features = features_pass_ratio[id]
    ...
    label_train = label_pass_ratio[id_train]
    label_test = label_pass_ratio[id_test]
...
def generate_dataset_local():
    ...
    save_dataset(features_train, "features_pass_ratio_train")

    save_dataset(features_test,  "features_pass_ratio_test")

    save_dataset(label_train, "label_pass_ratio_train")

    save_dataset(label_test,  "label_pass_ratio_test")
