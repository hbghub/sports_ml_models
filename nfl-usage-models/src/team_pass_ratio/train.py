...
from src.team_pass_ratio.utils import MODEL_VERSION, EXPERIMENT_NAME, hyperparams, featureNames, num_fields
...
if __name__ == "__main__":
    ...
    with mlflow.start_run() as run:
        ...
        features_train = load_dataset('features_pass_ratio_train')
        label_train = load_dataset('label_pass_ratio_train').values.ravel()
        ...
        features_test = load_dataset('features_pass_ratio_test')[featureNames]
        label_test = load_dataset('label_pass_ratio_test').values.ravel()

        evaluate_regression(model, features_train, label_train, features_test, label_test)

        # model = model_train(hyperparams, MODEL_VERSION, features_train, label_train)
        #
        # evaluate_regression(model, 'features_pass_ratio_train', 'label_pass_ratio_train',
        #                         'features_pass_ratio_test', 'label_pass_ratio_test')
