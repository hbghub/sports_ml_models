...
from src.player_rush_share.utils import MODEL_VERSION, EXPERIMENT_NAME, hyperparams, featureNames, num_fields, cat_fields
...
if __name__ == "__main__":
    ...
    with mlflow.start_run() as run:
        ...
        features_train = load_dataset('features_rush_share_train')
        label_train = load_dataset('label_rush_share_train').values.ravel()
        #pbp_df = load_dataset('pbp_df')

        features_train = features_train[featureNames]

        # feature adjustment for injury consideration
        id = features_train.ytd_rushingShareByPositionRankAdj > 0
        features_train.loc[id, 'ytd_rushingShareAdj'] = features_train.ytd_rushingShareByPositionRankAdj[id]
        features_train.drop(columns=['ytd_rushingShareByPositionRankAdj'], inplace=True)
        ...
        features_test = load_dataset('features_rush_share_test')[featureNames]
        label_test = load_dataset('label_rush_share_test').values.ravel()

        id = features_test.ytd_rushingShareByPositionRankAdj > 0
        features_test.loc[id,'ytd_rushingShareAdj'] = features_test.ytd_rushingShareByPositionRankAdj[id]
        features_test.drop(columns=['ytd_rushingShareByPositionRankAdj'], inplace=True)
